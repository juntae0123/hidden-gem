...
## 2''''. 저장소는 공개, 레시피는 비공개
- **파일을 만드는 순간에 '이게 공개돼도 되나'를 판단한다.** 나중에 빼려면 `git rm --cached` 로는 안 되고
  히스토리 재작성과 force push 가 필요하다. 실수 기록: 2026-09-07 공개 직전에 개인 작업 문서 몇 개를
  여러 커밋에서 걷어내야 했다. 사용자 지적 — "그런건 진작에 깃이그노어 했어야지". 맞다.
- 저장소 밖에 만들 것: 개인 작업 문서, 사업 문서(PRD·계획), 일일 브리핑·세션 보고, 고객사 자료.
  `Desktop/Hidden-Gem-비공개/` 에 **처음부터** 만든다. 저장소에 만들어야 할 이유가 있으면
  같은 커밋에 `.gitignore` 를 먼저 넣는다.
- **뺀 파일의 내용을 커밋 메시지·문서에 옮겨 적지 않는다.** 파일을 지워도 그 문장이 공개 히스토리에 남으면
  아무것도 뺀 게 아니다. 같은 날 내가 낸 실수 — 제외 사유를 적으면서 제외 대상의 문구를 인용했다.
  제외 커밋은 **무엇을 뺐는지가 아니라 어떤 범주를 뺐는지**만 적는다.
- 판단 기준 한 줄: **채용 담당자가 이 파일을 읽어도 손해가 없나.** 아니면 저장소 밖이다.
...
## 5'. UI 크기를 바꿀 때는 모바일 폭부터 본다
- "글씨가 너무 작다"는 지적을 받고 데스크톱 기준으로만 키웠다. 390px 화면에서 헤더가 터져
  로고가 두 줄, `랭킹` 이 `랭 / 킹` 으로 세로 분해됐다. 실수 기록: 2026-09-07 밤, 사용자 휴대폰 캡처로 발견.
- 규칙: 폰트·패딩을 키울 때는 **반응형 분기로** 키운다(`text-[13px] sm:text-[15px]`). 한 값으로 키우면
  가장 좁은 화면이 먼저 깨진다. 가로로 늘어서는 요소에는 `whitespace-nowrap` 을 같이 준다 —
  줄바꿈으로 깨지면 원인이 폭이라는 걸 알아보기 어렵다.
- 확인 기준 폭: **360 / 390 / 768 / 1280**. 좁은 쪽에서 넘치면 텍스트를 줄이기 전에 **덜 중요한 요소를 숨긴다**
  (로고 텍스트, 아이콘). 브랜드명은 히어로에 다시 나오므로 헤더에서 숨겨도 손실이 작다.
- 데스크톱 캡처만 보고 "고쳤다"고 하지 않는다. 화면 관련 변경은 사용자 폰 확인까지가 한 건이다.
