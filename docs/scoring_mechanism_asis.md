...
**1. X-Factor는 변별력이 아니라 상수 가산이다.** 평균 17.71 / 18점 만점, 거의 모든 게임이
상한에 포화됐다. 두 검토자는 "인기작에서 포화된다"고 예측했지만 실측은 더 극단적이다 —
**전원 포화**라서 순위에 영향을 주지 않고, 모든 점수를 18점 들어올리기만 한다.
예산 축소가 아니라 **제거**가 맞다(설명 문구 생성 용도로만 유지).

**2. Core의 변별력이 심각하게 부족하다.** σ 1.14, 폭 5.7점. 75점 예산에서 실제로 쓰이는 것이
그 정도다. "부분집합(4~10차원) 비교는 거리값이 양자화되어 변별력이 부족하다"는 지적이
실측으로 확인됐다. 시나리오별 폭도 2.3~5.1점.

**3. Gem이 취향 매칭보다 순위를 더 가르는 시나리오가 있다.** 힐링 프리셋에서 Gem σ 1.39 >
Core σ 0.86. 그런데 **그 Gem의 입력은 사용 불가 판정된 LLM `gem_potential` 기반 백분위**다
(근거 지수와 r=-0.022). 신뢰할 수 없는 값이 순위의 3분의 1을 결정하고 있다.
