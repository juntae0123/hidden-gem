...
검토 이력: 외부 검토 3건(TypingMind / Fable 5, GPT-5.6 Sol)과 그에 대한 판정은
`docs/external_review_log_0904.md` 에 따로 기록했다. 이 문서의 정정은 그 기록에서 나왔다.
...
flowchart TD
    ...
    U4["문장 검색<br/>POST /search/semantic"] --> P3
    P3 -->|"참조 게임 매칭됨<br/>('위쳐 같은 게임')"| P2

    P1["경로 A · score_v6.calculate_score_v6<br/>Core 75 + X-Factor 18 + Gem 6"]
    P2["경로 B · recommend_by_game<br/>지표 60% + 임베딩 40%"]
    P3["경로 C · semantic_search<br/>임베딩 85% + GPT 힌트 15%"]
    ...
    style P3 fill:#3e2d4a,color:#fff
```

**정정(외부 검토 지적, 소스 확인 완료)**: 이 문서 초판은 by-game 과 semantic 을 한 경로로 묶었다.
틀렸다. `semantic_search` 는 `raw_score = emb_score * 0.85 + hint_score * 0.15`(`recommender.py:1122`)
로 **완전히 다른 공식**이다. 즉 점수 경로는 **세 개**이고, 문장 검색에서 참조 게임이 매칭될 때만
경로 B로 우회한다.

| | 경로 A (취향 분석) | 경로 B (유사 게임) | 경로 C (문장 검색) |
|---|---|---|---|
| 점수 구성 | Core 75 + X-Factor 18 + Gem **6** | (지표 60% + 임베딩 40%) × 94 + Gem **5** | (임베딩 85% + 힌트 15%) × 94 + Gem **5** |
| 지표 비교 | 장르 핵심 + 의도 지표 **일부만** | 49개 **전부**, 4단계 가중 | GPT 힌트 지표만, **무가중** |
| 가중치 | 하드코딩 `2.0 / 1.0` | 4단계 `5.0 / 2.0 / 0.5 / 0.1` | **없음** (weight_map 계산 후 미사용) |
| 임베딩 | **안 함** | 40% | **85%** |
| Gem 공식 | discovery + quality + gem_signal | gem × 0.7 + review_bonus, × confidence | 경로 B와 동일 |
| 도달 가능 상한 | **90.1** | **88.8** | **99** (raw 1.0 가능) |
| 유명작 필터 | 인자 있음(무력) | 없음 | 없음 |

→ 같은 게임이 세 화면에서 다른 논리·다른 척도로 채점되는데 응답 필드명은 모두
`similarity_score` 다. 사용자는 같은 척도로 오해한다. `[D-11]`
→ 4단계 가중치는 **경로 A와 C 모두에서 죽어 있다.** 살아 있는 곳은 경로 B뿐이다. `[D-1]`
...
교사 코호트에 리뷰 실측이 없다는 사실이 `[D-3]` `[D-4]` `[D-5]` 를 동시에 유발한다.
(초판은 여기에 `[D-6]`을 넣었으나 D-6은 X-Factor 문제로 리뷰 결측과 무관하다 — 정정.)
...
→ 결정 필요: 4단계를 경로 A에 연결할지, query mask 방식으로 재설계할지.
   **"쿼리 의존 채점이 잘못됐다"는 뜻이 아니다.**

**주의 — "안전망은 `must_not` 이 한다"는 현재 코드로 성립하지 않는다.** `EXCLUSION_KEYWORDS`
(`recommender.py:215-227`)는 8개 지표뿐이고 트리거 단어("공포", "잔인", "노가다", "경쟁",
"멀티", "퍼마데스" 등)가 쿼리에 있어야만 발동한다. "힐링 게임"이라고만 치면 `must_not` 이
비어서 고어 9짜리 게임을 아무도 막지 않는다. query mask 로 가려면 (a) 의도 그룹의 방향성 있는
역방향 지표를 작은 가중치의 소프트 페널티로 유지하거나 (b) `must_not` 자동 유도(의도 그룹에서
역방향 지표 추출)를 함께 넣어야 한다. 마스크만 남기고 나머지를 0으로 하면 저차원 동점 문제가
돌아온다.

**경로 B의 4단계도 조건부다.** `recommend_by_game` 은 `query_hint` 가 있고 그것이 mood/feature
키워드 분류에 성공할 때만 차등 가중치를 쓴다. 힌트가 없으면 `group_name=None` → 전 지표
`W_NEUTRAL(0.5)` 균등 → 실질 무가중치다. 그런데 `query_hint` 는 `RecommendByGameRequest` 스키마에
없어 API로는 전달 불가(`routers/games.py:264` 가 `getattr` 로 읽지만 항상 None)이고, 내부
semantic 우회 경로만 힌트를 넘긴다. → **4단계가 실제로 작동하는 경우는 "문장 검색에서 참조
게임이 매칭된 뒤 by-game 으로 우회할 때"뿐이다.**
...
**단, '배선'이라는 표현에 딸린 경고(외부 검토 R2 지적, 수용)**: `build_preference_vector` 는
사용자가 말하지 않은 지표에도 `resolve_null(field, None)` 로 **목표값을 만들어 넣는다.**
사용자가 4개를 말했으면 나머지 45개에 대해 "중앙값이 목표"라는, 사용자가 하지 않은 진술이
점수에 들어간다. 가중치가 `W_NEUTRAL(0.5)` 로 낮아도 45개가 쌓이면 4개를 압도할 수 있다.
→ 그래서 R2 의 **질의 마스크**(말한 지표만 거리 계산에 넣고 나머지는 아예 제외) 쪽이
설계 의도 "점수는 사용자의 취향/검색문장에 따라 달라진다"와 더 맞다. 배선만 하고 끝내면
D-8·D-10 은 해결되지만 **말하지 않은 45개가 만드는 잡음은 남는다.**
...
## 7-2. 경로 C — 문장 검색 (별도 공식)

```mermaid
flowchart TD
    C1["쿼리 분석 GPT<br/>english_query · metric_hints · reference_game"] --> C2
    C2{"reference_game 이<br/>DB에 있나?"}
    C2 -->|예| C3["경로 B(by-game)로 우회<br/>기준 게임 제외 + 프랜차이즈 배제"]
    C2 -->|아니오| C4["쿼리 임베딩으로 pgvector 검색<br/>후보 limit x 2 만 가져옴"]
    C4 --> C5["must_not 필터 (파이썬)"]
    C5 --> C6["hint_score = 힌트별 근접도 평균<br/>match / len(metric_hints)"]
    C6 --> C7["raw = emb x 0.85 + hint x 0.15"]
    C7 --> C8["display = raw x 94 + gem x 5"]
```

경로 C의 특징 세 가지가 A·B와 다르다.
- **임베딩이 85%** 다. 지표는 GPT가 뽑아준 힌트 몇 개만, **가중치 없이** 근접도 평균으로 15%.
- `weight_map` 을 계산하지만 `hint_score` 계산에 쓰지 않는다 → 4단계 가중치가 여기서도 죽어 있다.
- raw 가 1.0 에 도달 가능하므로 **표시 상한 99가 유일하게 실제로 닿는 경로**다(D-7·D-15와 대조).
...
**교사 코호트에 대해 이 필터는 아무것도 걸러내지 못한다.** (학생 코호트는 리뷰 실측이
들어왔으므로 이후로는 일부 작동한다 — '한 번도 작동한 적 없다'는 초판 표현은 과했다.)
실측 검증: 취향 프리셋 5개 전부
...
## 11-2. 실측 — 상위 10 표본 안에서의 구성요소 편차 (해석 주의)

외부 검토의 지적("명목 최대점보다 실측 분산이 훨씬 중요하다")을 s0 스냅샷으로 계산했다.
취향 프리셋 10개 시나리오 × 상위 10 = 100건.

| 구성요소 | 명목 예산 | 평균 | **σ** | 범위 | 상한 포화 |
|---|---|---|---|---|---|
| Core | 75 | 62.43 | **1.14** | 59.0–64.7 | 상한 66.1 근처 |
| X-Factor | 18 | **17.71** | **0.59** | 15.6–18.0 | **거의 전원 18점** |
| Gem | 6 | 2.47 | **0.87** | 0.7–5.6 | — |
| 최종 | 99 | 82.61 | 1.14 | **80.9–85.9** | — |
...
### ⚠ 측정 한계 — 이 표를 "랭킹 지배력"으로 읽으면 안 된다 (자체 정정)

이 문서 초판은 위 σ 비율을 "실제 영향력 = 랭킹 지배력"이라고 적었다. **틀렸다.**
σ 비율(1.14 : 0.59 : 0.87을 합으로 나눈 것)은 순위 영향력의 척도가 아니고,
같은 문단에서 "X-Factor는 순위에 영향 없는 상수"라고 하면서 "순위의 22.6%를 결정"이라고
쓴 것은 자기모순이다.

**표본 오염도 있다.** σ_final(1.14)이 σ_core(1.14)와 정확히 같다. `final = Core+XF+Gem` 이므로
`Var(final) = ΣVar + 2ΣCov` 인데, Var 합이 2.40이고 Var(final)이 1.30이니 **ΣCov ≈ -0.55**다.
이 강한 음의 상관은 게임의 성질이 아니라 **"합계 상위 10을 뽑았다"는 선택이 만든 인공물**
(collider 효과)이다. 따라서:
- 상위 10의 σ는 전체 후보 풀의 변별력을 **과소평가**한다
- "Core 실사용 폭 5.7점"은 Core의 변별력이 아니라 **승자들 사이의 폭**이다
- "힐링에서 Gem σ > Core σ" 비교도 같은 오염을 받는다

**오염과 무관하게 유효한 결론 하나**: 사용자가 보는 표시 점수가 80.9~85.9에 몰려 구별되지
않는다. 그건 정의상 상위 10의 성질이므로 절단 효과와 무관하다.

**제대로 재려면**: 구성요소를 하나씩 0으로 놓고 **전체 후보를 다시 정렬**한 뒤 원래 순위와의
Kendall τ / RBO를 재는 절제(ablation) 재랭킹이 맞다. 스냅샷(상위 10)만으로는 불가능하고
채점 쪽에서 전체 풀을 다시 돌려야 한다. 그 전까지 "Gem이 순위의 1/3을 쥐고 있다"는 주장은
**보류**한다.

### 강한 가설 — X-Factor는 상수가 아니라 선택 게이트일 가능성이 크다

**표기 강등(2026-09-04 재검토)**: 초판은 이걸 "오염되지 않은 관찰"로 적었다. 과했다.
상위 10 표본만으로는 게이트 작동을 **증명할 수 없다** — 절제 실험(전체 풀 재정렬)으로만
확인된다. 아래는 **강한 가설**로 읽어야 한다.

상위 10이 전원 ~18점이라는 사실의 더 그럴듯한 해석은 "아무것도 안 한다"가 아니라
**"선택 게이트로 작동한다"**는 것이다. Core의 승자 폭이 5.7점뿐이므로, X-Factor가
15.6 미만인 게임은 Core로 만회할 수 없어 **상위권 진입 자체가 막힌다.** 즉 지금 X-Factor는
"8 이상 지표가 많은 게임"만 통과시키는 필터다.

**보류 항목**: "8 이상 지표가 많다 = 완성도 높은 인기작"이라는 연결은 **측정하지 않았다.**
X-Factor 값과 리뷰 수/인기도의 상관은 아직 재본 적이 없으므로 이 주장은 보류한다.
(재려면: 전체 풀에서 X-Factor 값 vs review_count 상관을 직접 계산) 설계 의도 3(유명작 상위 금지)과 충돌하는 지점이 여기다.

**제거 결론은 같지만 예측이 달라진다:**
- "무해한 상수"라면 → 제거 후 diff 에서 순위 변화가 거의 없어야 한다
- "선택 게이트"라면 → **상위 10 구성이 상당히 바뀌어야 한다** (극단값 밀도가 낮은 게임 진입)

전환 계획서 5장의 합격 기준("취향 시나리오 상위 10 절반 이상 유지")은 이 경우 **오판을
유발한다.** X-Factor 제거 커밋에서는 순위가 크게 움직이는 것이 **진단의 확인**이다.

**그리고 단독 제거는 위험하다.** 제거하면 `final = Core + Gem` 이 되는데, 상위권에서 Core
변별이 좁은 현재 상태로는 Gem의 상대 비중이 오히려 커진다. 그 Gem 입력은 신뢰 불가
판정(r=-0.022)이다. → X-Factor 제거는 **Core 변별력 개선(sigmoid 정규화 + query mask) 또는
Gem 입력 교체와 같은 변경 창**에서 해야 한다. 나누면 중간 상태가 지금보다 나빠진다.

**부수 결과**: 표시 점수가 80.9~85.9 에 몰려 있다. 99점 척도인데 사용자가 보는 매치율이
전부 82~85%라 구별되지 않는다. UX 문제이기도 하다.

재현: `python -m embeddings.rec_snapshot --variance s0_before_gate`
...
| D-11 | 세 경로의 Gem 예산 불일치 (A=6.0, B·C=×5.0) | `score_v6.py:28`, `recommender.py:500` | 화면 간 점수 비교 불가 |
...
| D-20 | **가중치가 값에 곱해져 거리에서 제곱된다.** `vec[i] = val * w` 후 `norm(v1-v2)` → 기여도가 `w²`. primary 5.0 vs irrelevant 0.1 은 50배가 아니라 **2,500배**. 주석·문서는 "5x"라고 적혀 있다 | `recommender.py:439, 455` | 의도한 강조 배율과 실제가 다르다. 권장: `sqrt(Σ w·diff²/Σw)` (높음) |
| D-21 | `n_active` 가 **양수 가중치 개수**를 센다 — 0.1과 5.0을 같은 1차원으로 취급. 경로 B는 항상 49, 경로 A도 그룹 미지정 시 49 | `recommender.py:765, 849` | 쿼리마다 유효 가중치 질량이 달라도 정규화가 안 된다 (중간) |
| D-22 | 경로 C `hint_score = match / len(metric_hints)` — 유효하지 않거나 값이 NULL이라 **건너뛴 힌트도 분모에 남는다** | `recommender.py:1120` | 힌트 일부가 스킵되면 점수가 부당하게 낮아진다 (중간) |
