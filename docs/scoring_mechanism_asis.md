...
## 11-2. 실측 — 명목 예산과 실제 랭킹 지배력이 다르다 (가장 중요)
...
### 오염되지 않은 관찰 — X-Factor는 상수가 아니라 선택 게이트다

상위 10이 전원 ~18점이라는 사실의 올바른 해석은 "아무것도 안 한다"가 아니라
**"강력한 선택 게이트로 작동한다"**는 것이다. Core의 승자 폭이 5.7점뿐이므로, X-Factor가
15.6 미만인 게임은 Core로 만회할 수 없어 **상위권 진입 자체가 막힌다.** 즉 지금 X-Factor는
"8 이상 지표가 많은 게임"(= LLM이 후하게 채점한, 대체로 완성도 높은 게임)만 통과시키는
필터다. 설계 의도 3(유명작 상위 금지)과 충돌하는 지점이 여기다.
