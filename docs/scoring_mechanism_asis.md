...
| `review_count` | **비어 있음(NULL/0)** | 조회 중 (완료 시 전건) |
| `steam_positive_ratio` | **비어 있음** | 조회 중 |
| `embedding` | 있음 | 있음 |
...
`[D-1]` `recommend_by_preference` 는 4단계 `weight_map` 과 `target_vec`·`cand_vec` 을 계산하지만
**점수에 쓰지 않는다**(응답 echo·match_reason 정렬 전용). 점수는 위 세 항이 전부다.
→ "검색 의도에 따라 지표를 5.0/2.0/0.5/0.1로 분류한다"는 서술은 이 경로에서 사실이 아니다.
...
| # | 내용 | 위치 | 영향 |
|---|---|---|---|
| D-1 | 4단계 가중치가 취향 분석 점수에 **반영되지 않음** (계산 후 미사용) | `recommender.py:846-899` | 핵심 기능이 메인 동선에서 무효 |
...
| D-4 | "리뷰 없음"과 "리뷰 0"을 구분 못 함 | `score_v6.py:292`, `recommender.py:566` | 교사 게임이 발굴 보너스를 부당 수령 |
| D-5 | `max_review_count` 가 NULL을 통과시켜 무력 + 3개 경로에 미적용 | `recommender.py:858-862`, `routers/games.py:391` | 유명작 필터가 작동한 적 없음 |
| D-6 | X-Factor가 49지표를 무가중치 순회, 신뢰 불가 지표 포함 | `score_v6.py:232-246` | 지표 1개로 +3.9점, 3개면 +11.7점 |
| D-7 | sigmoid 독스트링 오류, 실제 최대 점수 90.1 (99 아님) | `score_v6.py:123-135` | 점수 상단이 도달 불가 |
| D-8 | 부정 선호(값 0)가 Core에서 무시됨 (`v >= 7` 조건) | `score_v6.py:172-175` | "공포 빼고"가 반영 안 됨 |
...
| D-12 | 가중 벡터에 코사인 → 0.95+ 상수화 | `recommender.py:487`, `score_v6.py:200` | 유사도 33~35%가 변별력 없음 |
...
### 검토자에게 부탁할 질문
1. D-1: 4단계 가중치를 경로 A에 실제로 연결할 것인가, 아니면 문서·포트폴리오 주장을 정정할 것인가?
