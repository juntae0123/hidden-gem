...
| 3 서비스 확인 | 0 | 10분 |
| 4 재개 (남은 3/16~6월초 ≈ 5,500건) | **배치 $27** (실단가 $0.0049/건, 대시보드 검증) | 회차당 30~60분 |
...
| 방식 | 요청당 (대시보드 검증) | 5,500건 | 품질 |
|---|---|---|---|
| **학생 배치 (권장)** | **$0.0049** | **$27** | 49지표 교사급(MAE 0.77), gem은 근거 기반으로 대체 |
| 학생 동기+캐시 | $0.0099 (배치의 2배) | $55 | 배치와 동일 — 비쌀 이유가 없다 |
| 교사 배치 (few-shot 불필요) | 정가 재확인 필요 | 참고용 | 기준 그 자체 |

배치에도 캐시 할인이 적용되는 것이 확인됐다(Cost 탭 `batch api | cached input`).
따라서 배치가 항상 싸다 — 재개는 지금까지와 같은 배치 방식으로 한다.

재개 명령:
```
docker compose exec batch rm /app/data/STOP_BACKFILL
docker compose exec -d batch sh -c "python -m embeddings.weekly_pipeline --from 2026-03-16 --to 2026-06-10 --limit 500 --loop --wait-timeout 60 --audit-n 30 > /app/data/backfill2.log 2>&1"
```
멈추기는 언제든 `data/STOP_BACKFILL` 파일 생성(컨테이너 밖에서 파일만 만들어도 됨).
`--audit-n 30` 은 루프 종료 후 품질 드리프트 점검(교사 30건 재분석, 약 $0.15).
